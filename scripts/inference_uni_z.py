...
def ldm_cond_sample_dataset(config_path, ckpt_path, dataset, batch_size):
    ...
    with torch.no_grad():
        ...
        prompt = ["a very red polyp with blood"] * batch_size
        text = clip.encode(prompt)
        text = text.to(seg.device)


        samples, _ = model.sample_log(cond=seg,text = None, batch_size=batch_size, ddim=True,
                                      ddim_steps=200,unconditional_guidance_scale=7.5,
                                                         unconditional_conditioning=uc,                    
                                                         eta=1.0,multi = False)

        samples = model.decode_first_stage(samples)
    results_dir = '/home/hyl/yujia/test_3_15'
    if not os.path.exists(results_dir):
        os.makedirs(results_dir)
    save_image((real+1.0)/2.0, os.path.join(results_dir, 'real_4.png'))
    save_image((samples+1.0)/2.0, os.path.join(results_dir, 'mask.png'))
...
#         results_dir = '/home/hyl/yujia/FID_test/data_test_no_cla'
#         if not os.path.exists(results_dir):
#             os.makedirs(results_dir)
#         save_image((samples + 1.0) / 2.0, os.path.join(results_dir, f'{i + 1}.png'))

#         if i + 1 == 100:  # 生成100张图片后结束循环
#             break
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt_path', type=str, default='/home/hyl/yujia/conditional-polyp-diffusion/latent-diffusion/mask_refine/2024-03-13T11-34-48_mask_refine/checkpoints/trainstep_checkpoints/epoch=1-step=599.ckpt')
