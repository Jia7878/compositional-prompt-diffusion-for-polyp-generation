...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb,context=None,text = None):
        for layer in self:
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps=None, context=None,text = None, y=None,**kwargs):
        """
        ...
        for module in self.input_blocks:
            h = module(h, emb, context,text)
            hs.append(h)
        h = self.middle_block(h, emb, context,text)
        ...
        for module in self.output_blocks:
            ...
            if self.enable_freeu:
                ...
                #     # h[:,:320] = h[:,:320] * ((self.b2 - 1 ) * hidden_mean + 1)
                #     h[:,:320] = h[:,320] * 2.0
                #     hs_ = Fourier_filter(hs_, threshold=1, scale=0.5)
            # ---------------------------------------------------------
